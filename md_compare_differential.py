...
try:
    ...
    print("Warning: scipy not available. Statistical analysis features will be limited.")

from md_compare_core import MDSimulation, AnalysisConfig, OutputManager, MDComparator
...
class DynamicsComparison:
    ...
    significant_correlation_changes: List[Tuple[str, str, float, float]]  # (res1, res2, old_corr, new_corr)
...
class DifferentialAnalyzer:
    ...
    def __init__(self, config: DifferentialConfig = None, output_dir: str = "differential_analysis_results"):
        """
    ...
    def run_differential_analysis(self, 
                                sim1_topology: str, sim1_trajectory: str, sim1_name: str,
                                sim2_topology: str, sim2_trajectory: str, sim2_name: str,
                                analysis_config: AnalysisConfig = None) -> ComprehensiveDifferentialResults:
        """
        ...
        sim1_results, sim2_results = self._run_individual_analyses(
            sim1_topology, sim1_trajectory, sim1_name,
            sim2_topology, sim2_trajectory, sim2_name,
            analysis_config
        )
    ...
    def _run_individual_analyses(self, 
                               sim1_topology: str, sim1_trajectory: str, sim1_name: str,
                               sim2_topology: str, sim2_trajectory: str, sim2_name: str,
                               analysis_config: AnalysisConfig) -> Tuple[MDSimulation, MDSimulation]:
    ...
    def _run_comparative_analyses(self, sim1: MDSimulation, sim2: MDSimulation) -> Dict[str, Any]:
        ...
        if self.config.compare_dynamics:
            ...
            comparison_results['dynamics'] = self.dynamics_comparator.compare_dynamics(
                getattr(sim1, 'dynamic_analysis', None),
                getattr(sim2, 'dynamic_analysis', None)
            )
    ...
    def _compile_comprehensive_results(self, sim1: MDSimulation, sim2: MDSimulation, 
                                     comparison_results: Dict[str, Any], 
                                     sim_names: Tuple[str, str]) -> ComprehensiveDifferentialResults:
    ...
    def _identify_most_significant_changes(self, comparison_results: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _generate_executive_summary(self, comparison_results: Dict[str, Any], 
                                  sim_names: Tuple[str, str]) -> Dict[str, Any]:
    ...
    def _create_dynamics_csv_reports(self, dynamics_comparison):
        ...
        if dynamics_comparison.significant_correlation_changes:
            corr_df = pd.DataFrame(dynamics_comparison.significant_correlation_changes, 
                                 columns=['Residue1', 'Residue2', 'Original_Correlation', 'New_Correlation'])
            corr_df['Correlation_Change'] = corr_df['New_Correlation'] - corr_df['Original_Correlation']
            corr_df['Change_Magnitude'] = corr_df['Correlation_Change'].abs()
            corr_df['Change_Type'] = corr_df['Correlation_Change'].apply(lambda x: 'increase' if x > 0 else 'decrease')
    ...
    def _create_kinetics_csv_reports(self, kinetics_comparison):
        ...
        if kinetics_comparison.timescale_changes.size > 0:
            timescale_df = pd.DataFrame({
                ...
                'Change_Type': ['increase' if x > 0 else 'decrease' for x in kinetics_comparison.timescale_changes]
            })
        ...
        similarity_df = pd.DataFrame([
            {
                ...
                'Similarity_Category': 'high' if score > 0.8 else 'medium' if score > 0.5 else 'low'
            }
    ...
    def _create_visualizations(self, results):
        ...
        try:
            ...
            if results.dynamics_comparison and results.dynamics_comparison.dccm_difference_matrix.size > 0:
    ...
    def _create_dccm_difference_plot(self, dynamics_comparison):
        ...
        # Create heatmap
        sns.heatmap(dynamics_comparison.dccm_difference_matrix, 
                   cmap=self.config.heatmap_colormap, center=0,
        ...
        # Save plot
        dynamics_dir = self.subdirs['dynamics']
        plt.savefig(dynamics_dir / 'dccm_difference_heatmap.png', 
    ...
    def _create_summary_plots(self, results):
        ...
        if results.overall_similarity_scores:
            ...
            reports_dir = self.subdirs['reports']
            plt.savefig(reports_dir / 'similarity_scores_summary.png',
                       dpi=self.config.figure_dpi, bbox_inches='tight')
            plt.close()
...
<body>
    ...
    <div class="section">
        ...
        with open(reports_dir / 'comprehensive_differential_report.html', 'w', encoding='utf-8') as f:
...
class NetworkComparator:
    ...
    def compare_networks(self, net1, net2) -> NetworkComparison:
        ...
        nodes_added, nodes_removed, edges_added, edges_removed, edge_weight_changes = self._compare_topology(graph1, graph2)
    ...
    def _compare_topology(self, graph1, graph2) -> Tuple[List[str], List[str], List[Tuple], List[Tuple], Dict]:
        ...
        try:
            ...
            print("Warning: NetworkX not available for topology comparison")
            return [], [], [], [], {}
    ...
    def _compare_centralities(self, net1, net2) -> Tuple[Dict[str, Dict[str, float]], Dict[str, List[str]]]:
        ...
        for metric in centrality_metrics:
            ...
            if metric_changes:
                ...
                large_changes = [node for node, change in metric_changes.items() 
    ...
    def _compare_communities(self, net1, net2) -> Tuple[Dict[str, Any], float]:
        ...
        community_changes = {
            ...
            'avg_community_size_change': (np.mean(sizes2) - np.mean(sizes1)) if sizes1 and sizes2 else 0,
            'community_size_variance_change': (np.var(sizes2) - np.var(sizes1)) if sizes1 and sizes2 else 0
        }
        
        print(f"    Community changes - Modularity: {modularity_change:+.4f}, "
    ...
    def _create_empty_network_comparison(self) -> NetworkComparison:
        ...
        return NetworkComparison(
            ...
            edge_weight_changes={}, centrality_changes={}, centrality_ranking_changes={},
            community_changes={}, modularity_change=0.0, network_property_changes={},
            statistical_significance={}
        )
...
class DynamicsComparator:
    ...
    def compare_dynamics(self, dyn1, dyn2) -> DynamicsComparison:
        ...
        dccm_diff_matrix, correlation_changes, regional_changes = self._compare_dccm_matrices(dyn1, dyn2)
        
        # Compare PCA results
        pca_variance_changes, eigenvector_similarities, pc_shifts = self._compare_pca_results(dyn1, dyn2)
    ...
    def _compare_dccm_matrices(self, dyn1: Dict, dyn2: Dict) -> Tuple[np.ndarray, List[Tuple], Dict]:
    ...
    def _analyze_regional_correlation_changes(self, dccm_diff: np.ndarray, 
                                            dyn1: Dict, dyn2: Dict) -> Dict[str, Dict[str, float]]:
    ...
    def _perform_correlation_statistics(self, dyn1: Dict, dyn2: Dict, 
                                      dccm_diff: np.ndarray) -> Tuple[np.ndarray, List[Tuple], Dict]:
        ...
        print(f"    Effect sizes - Large: {effect_sizes['large_effects']}, "
...
class EnergeticsComparator:
    ...
    def _compare_energy_minima(self, landscape1: Dict, landscape2: Dict) -> List[Dict[str, Any]]:
        ...
        for i, min1 in enumerate(minima1[:5]):  # Top 5 minima
            ...
            for j, min2 in enumerate(minima2):
            ...
            if best_match and min_distance < 2.0:  # Within reasonable distance
                ...
                minima_comparison.append({
                    ...
                    'population_change': best_match.get('population', 0) - min1.get('population', 0)
                })
        
        return minima_comparison
    
    def _compare_energy_barriers(self, landscape1: Dict, landscape2: Dict) -> List[Dict[str, float]]:
        ...
        for i, barrier1 in enumerate(barriers1):
            if i < len(barriers2):
                ...
                barrier_changes.append({
                    ...
                    'rate_change_factor': np.exp(-height_change) if abs(height_change) < 10 else float('inf')
                })
    ...
    def _calculate_free_energy_changes(self, landscape1: Dict, landscape2: Dict) -> Dict[str, float]:
        """Calculate free energy changes"""
        
        # Extract temperature
        temp = landscape1.get('temperature', 310.0)  # Default 310K
        kT = 0.001987 * temp  # Boltzmann constant in kcal/mol/K
    ...
    def _create_empty_energetics_comparison(self) -> EnergeticsComparison:
        ...
        return EnergeticsComparison(
            energy_difference_surface=None, minima_comparison=[], barrier_height_changes=[],
            stability_changes={}, conformational_state_changes={}, free_energy_changes={},
            energy_change_significance={}, conformational_population_changes={}
        )
...
class KineticsComparator:
    ...
    def _compare_timescales(self, msm1: Dict, msm2: Dict) -> Tuple[np.ndarray, np.ndarray, float]:
        ...
        print(f"    Dominant timescale change: {dominant_change:.2f} (ratio: {timescale_ratios[0]:.2f})")
    ...
    def _compare_transition_fluxes(self, msm1: Dict, msm2: Dict) -> np.ndarray:
        ...
        if transition_matrix1.shape != transition_matrix2.shape:
            print(f"    Warning: Transition matrix size mismatch: {transition_matrix1.shape} vs {transition_matrix2.shape}")
            return np.array([])
    ...
    def _compare_pathway_probabilities(self, msm1: Dict, msm2: Dict) -> Dict[str, float]:
        ...
        # Sort by magnitude of change
        pathway_changes = dict(sorted(pathway_changes.items(), 
            ...
            for i in range(min_states):
                if pop1[i] > 0:
                    ...
                    stability_changes[i] = {
                        ...
                        'stability_change': 'stabilized' if relative_change > 0.1 else 'destabilized' if relative_change < -0.1 else 'unchanged'
                    }
        ...
        if 'critical_transitions' in msm1 and 'critical_transitions' in msm2:
            ...
            for i, (trans1, trans2) in enumerate(zip(transitions1, transitions2)):
                ...
                critical_pathways.append({
                    ...
                    'impact': 'accelerated' if rate_change > 0 else 'decelerated' if rate_change < 0 else 'unchanged'
                })
    ...
    def _analyze_kinetic_bottlenecks(self, msm1: Dict, msm2: Dict) -> Dict[str, float]:
        ...
        if timescales1.size > 0 and timescales2.size > 0:
            ...
            mean_change = np.mean(timescales2[:min(5, len(timescales2))]) - np.mean(timescales1[:min(5, len(timescales1))])
    ...
    def _create_empty_kinetics_comparison(self) -> KineticsComparison:
        ...
        return KineticsComparison(
            timescale_changes=np.array([]), timescale_ratios=np.array([]),
            dominant_timescale_change=0.0, transition_flux_changes=np.array([]),
            pathway_probability_changes={}, metastable_state_population_changes=np.array([]),
            metastable_state_stability_changes={}, critical_pathway_changes=[],
            bottleneck_analysis={}
        )
...
class AllostericComparator:
    ...
    def compare_allosteric(self, metrics1, metrics2) -> AllostericComparison:
        ...
        if not (hasattr(metrics1, 'allosteric_pathways') and hasattr(metrics2, 'allosteric_pathways')):
    ...
    def _compare_efficiency_matrices(self, ap1: Dict, ap2: Dict) -> Tuple[np.ndarray, Dict[str, float]]:
    ...
    def _analyze_pathway_disruption(self, ap1: Dict, ap2: Dict) -> List[Dict[str, Any]]:
        ...
        for i, pathway1 in enumerate(pathways1[:10]):  # Top 10 pathways
            ...
            for j, pathway2 in enumerate(pathways2):
            ...
            if best_match and best_similarity > 0.5:  # Reasonable match
                ...
                disruption_analysis.append({
                    ...
                    'efficiency_change': best_match.get('efficiency', 0.0) - pathway1.get('efficiency', 0.0),
                    'disruption_score': disruption_score,
                    'pathway_similarity': best_similarity,
                    'length_change': len(best_match.get('path', [])) - len(pathway1.get('path', [])),
                    'significance': 'high' if abs(disruption_score) > 0.3 else 'medium' if abs(disruption_score) > 0.1 else 'low'
                })
    ...
    def _compare_hotspot_rankings(self, ap1: Dict, ap2: Dict) -> List[Dict[str, Any]]:
        ...
        for node in all_nodes:
            ...
            rank1 = next((i for i, hs in enumerate(hotspots1) if hs['node'] == node), len(hotspots1))
            rank2 = next((i for i, hs in enumerate(hotspots2) if hs['node'] == node), len(hotspots2))
            ...
            if abs(score_change) > 0.05 or abs(rank_change) > 2:  # Significant changes
                ranking_changes.append({
                    ...
                    'significance': 'high' if abs(score_change) > 0.2 else 'medium' if abs(score_change) > 0.1 else 'low'
                })
            ...
            for j, chain2 in enumerate(chains):
                if i < j:  # Avoid duplicate pairs
                    ...
                    cross_chain_changes[f"{chain1}_{chain2}"] = {
                        ...
                        'correlation_change': np.corrcoef(submatrix1.flatten(), submatrix2.flatten())[0,1]
                    }
