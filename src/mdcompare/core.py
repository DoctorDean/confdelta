...
#
# PyEMMA is heavy and is *not* imported at module-import time; doing so was a
# source of import-time print spam and slow imports. We only record whether a
# backend is installed here, and import it lazily inside the MSM routines.
...
PYEMMA_AVAILABLE = _module_available("pyemma")
DEEPTIME_AVAILABLE = _module_available("deeptime")
    ...
    def _resolve_chain_labels(self) -> np.ndarray:
        ...
        try:
            ...
            if len(seg_ids) == n and any(str(s).strip() for s in seg_ids):
                ...
                return np.array(
                    [str(s).strip() or "A" for s in seg_ids], dtype=object
                )
        except (AttributeError, mda.exceptions.NoDataError):
            pass
        ...
        try:
            ...
            if len(seg_idx) == n and len(np.unique(seg_idx)) > 1:
                return np.array(
                    [chr(ord("A") + int(i) % 26) for i in seg_idx], dtype=object
                )
        except (AttributeError, mda.exceptions.NoDataError):
            pass
        ...
        logger.info(
            "%s: no chain/segment information found; treating system as a "
            "single chain 'A'.",
            self.name,
        )
        return np.array(["A"] * n, dtype=object)
    ...
    def _setup_residue_mapping(self):
        ...
        for chain in seen:
            ...
            self._chain_info[chain] = {
                ...
                "residue_range": (
                    (int(chain_resids[0]), int(chain_resids[-1]))
                    if len(chain_resids)
                    else (0, 0)
                ),
            }
...
class NetworkAnalyzer:
    ...
    def compute_dynamic_analysis(self, simulation: MDSimulation) -> Dict[str, Any]:
        ...
        if self.config.compute_msm and PYEMMA_AVAILABLE:
            ...
            logger.warning("No MSM backend available; skipping MSM analysis. Install with: pip install 'md-compare[msm]'")
    ...
    def _perform_msm_clustering(self, features: np.ndarray):
        ...
        import pyemma.coordinates as coor  # lazy import: heavy optional dep
    ...
    def _build_msm_model(self, discrete_trajectory: np.ndarray) -> Tuple[Any, np.ndarray, List[np.ndarray]]:
        ...
        import pyemma.msm as msm  # lazy import: heavy optional dep
        try:
