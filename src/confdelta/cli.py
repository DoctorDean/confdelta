    ...
    try:
        ...
        if "comparative_analysis" in results:
            ...
            if "network_properties" in comp_analysis:
                print("  Network property comparison: Available")
            if "centrality_measures" in comp_analysis:
                print("  Centrality comparison: Available")
            if "contact_patterns" in comp_analysis:
                print("  Contact pattern comparison: Available")
...
def run_comprehensive_differential_analysis(args):
    ...
    try:
        ...
        print(
            f"Differential results: {args.output}/02_network_comparisons/ through 06_allosteric_comparisons/"
        )
...
def run_differential_analysis(args):
    ...
    print(
        "Note: For comprehensive differential analysis, use 'differential' command instead of 'diff'"
    )
    ...
    try:
        ...
        md_compare.run_comparison([args.sim1, args.sim2])
        monitor.end_step()
        ...
        if differential_results["n_increased"] > 0:
            ...
            print(
                f"  Top increase: {top_increase['residue_pair']} (+{top_increase['difference']:.3f})"
            )

        if differential_results["n_decreased"] > 0:
            top_decrease = differential_results["decreased_contacts"][0]
            print(
                f"  Top decrease: {top_decrease['residue_pair']} ({top_decrease['difference']:.3f})"
            )
        ...
        "simulations": [
            {
                ...
                "metadata": {"temperature": 300, "pressure": 1.0, "force_field": "CHARMM36"},
            },
            {
                "name": "mutant",
...
For more information, visit: https://github.com/DoctorDean/confdelta
    ...
    # Single simulation analysis
    single_parser = subparsers.add_parser("single", help="Analyze a single simulation")
    single_parser.add_argument(
        "-t", "--topology", required=True, help="Topology file (PDB, GRO, etc.)"
    )
    single_parser.add_argument(
        "-x", "--trajectory", required=True, help="Trajectory file (XTC, DCD, etc.)"
    )
    ...
    single_parser.add_argument(
        "--threshold", type=float, default=0.2, help="Contact frequency threshold for network edges"
    )
    single_parser.add_argument(
        "--interaction-types",
        nargs="+",
    ...
    single_parser.add_argument(
        "--timeout", type=int, default=300, help="Timeout for expensive computations (seconds)"
    )
    single_parser.add_argument(
        "--no-preprocess", action="store_true", help="Disable MD preprocessing"
    )
    single_parser.add_argument(
        "--align-selection", default="name CA", help="Selection for structural alignment"
    )
    single_parser.add_argument(
        "--center-selection", default="protein", help="Selection for centering"
    )
    single_parser.add_argument(
        "--compute-dccm",
        action="store_true",
    ...
    single_parser.add_argument(
        "--no-dccm", dest="compute_dccm", action="store_false", help="Skip DCCM computation"
    )
    single_parser.add_argument(
        "--compute-pca",
        action="store_true",
    ...
    single_parser.add_argument(
        "--no-pca", dest="compute_pca", action="store_false", help="Skip PCA computation"
    )
    single_parser.add_argument(
        "--pca-components",
        type=int,
    ...
    single_parser.add_argument(
        "--no-communities",
        dest="compute_communities",
        action="store_false",
        help="Skip community detection",
    )
    single_parser.add_argument(
        "--no-paths", dest="compute_paths", action="store_false", help="Skip detailed path analysis"
    )
    single_parser.add_argument(
        "--no-allosteric",
        dest="allosteric_analysis",
        action="store_false",
    ...
    single_parser.add_argument(
        "--msm-stride", type=int, default=1, help="Stride for coordinate extraction (default: 1)"
    )
    single_parser.add_argument(
        "--msm-features",
        default="distances",
    ...
    single_parser.add_argument(
        "--no-kinetics", dest="compute_kinetics", action="store_false", help="Skip kinetic analysis"
    )
    single_parser.add_argument(
        "--kinetic-timescales",
        type=int,
    ...
    single_parser.add_argument(
        "--no-metastable",
        dest="compute_metastable_states",
        action="store_false",
    ...
    # Multiple simulation comparison
    compare_parser = subparsers.add_parser("compare", help="Compare multiple simulations")
    ...
    compare_parser.add_argument(
        "--threshold", type=float, default=0.2, help="Contact frequency threshold"
    )
    compare_parser.add_argument(
        "--interaction-types",
        nargs="+",
    ...
    compare_parser.add_argument(
        "--segments", type=int, default=5, help="Number of trajectory segments"
    )
    compare_parser.add_argument(
        "--timeout", type=int, default=300, help="Timeout for expensive computations (seconds)"
    )
    ...
    diff_parser = subparsers.add_parser(
        "diff", help="Differential analysis between two simulations"
    )
    diff_parser.add_argument(
        "-t1", "--topology1", required=True, help="Topology file for first simulation"
    )
    diff_parser.add_argument(
        "-x1", "--trajectory1", required=True, help="Trajectory file for first simulation"
    )
    diff_parser.add_argument("-n1", "--sim1", required=True, help="Name for first simulation")
    diff_parser.add_argument(
        "-t2", "--topology2", required=True, help="Topology file for second simulation"
    )
    diff_parser.add_argument(
    ...
    differential_parser.add_argument(
        "-n1", "--sim1", required=True, help="Name/identifier for first simulation"
    )
    differential_parser.add_argument(
        "-t2", "--topology2", required=True, help="Topology file for second simulation"
    )
    ...
    differential_parser.add_argument(
        "-n2", "--sim2", required=True, help="Name/identifier for second simulation"
    )
    differential_parser.add_argument(
        "-o",
        "--output",
    ...
    differential_parser.add_argument(
        "--no-network-comparison", action="store_true", help="Skip network topology comparison"
    )
    differential_parser.add_argument(
        "--no-dynamics-comparison", action="store_true", help="Skip dynamics (DCCM/PCA) comparison"
    )
    differential_parser.add_argument(
    ...
    differential_parser.add_argument(
        "--no-kinetics-comparison", action="store_true", help="Skip kinetics (MSM) comparison"
    )
    differential_parser.add_argument(
        "--no-allosteric-comparison",
        action="store_true",
    ...
    differential_parser.add_argument(
        "--no-plots", action="store_true", help="Skip difference plot generation"
    )
    differential_parser.add_argument(
        "--publication-figures",
        action="store_true",
    ...
    differential_parser.add_argument(
        "--figure-dpi", type=int, default=300, help="Figure resolution in DPI (default: 300)"
    )

    # Output options
    differential_parser.add_argument(
        "--no-html-report", action="store_true", help="Skip HTML report generation"
    )
    differential_parser.add_argument(
        "--excel-export", action="store_true", help="Export results to Excel workbook"
    )
    ...
    differential_parser.add_argument(
        "--cutoff", type=float, default=4.5, help="Contact distance cutoff in Å (default: 4.5)"
    )
    differential_parser.add_argument(
        "--threshold", type=float, default=0.2, help="Contact frequency threshold (default: 0.2)"
    )
    differential_parser.add_argument(
        "--community-method",
        default="leiden",
    ...
    differential_parser.add_argument(
        "--allosteric-sources", nargs="+", help="Allosteric source residues (e.g., A_50 B_50)"
    )
    differential_parser.add_argument(
        "--allosteric-targets", nargs="+", help="Allosteric target residues (e.g., A_25 B_25)"
    )
    differential_parser.add_argument(
        "--msm-lag-time", type=int, default=10, help="MSM lag time in frames (default: 10)"
    )
    differential_parser.add_argument(
        "--msm-clusters", type=int, default=100, help="Number of MSM clusters (default: 100)"
    )
    differential_parser.add_argument(
        "--landscape-temp",
        type=float,
    ...
    differential_parser.add_argument(
        "--landscape-bins", type=int, default=50, help="Energy landscape bins (default: 50)"
    )
    ...
    differential_parser.add_argument(
        "--segments", type=int, default=5, help="Number of trajectory segments (default: 5)"
    )
    differential_parser.add_argument(
        "--timeout",
        type=int,
    ...
    differential_parser.add_argument(
        "--center-selection", default="protein", help='Selection for centering (default: "protein")'
    )
    differential_parser.add_argument(
        "--compute-dccm",
        action="store_true",
    ...
    differential_parser.add_argument(
        "--no-paths", dest="compute_paths", action="store_false", help="Skip path analysis"
    )
    differential_parser.add_argument(
        "--allosteric-analysis",
        action="store_true",
    ...
    differential_parser.add_argument(
        "--no-msm", dest="compute_msm", action="store_false", help="Skip MSM computation"
    )
    differential_parser.add_argument(
        "--msm-stride",
        type=int,
    ...
    differential_parser.add_argument(
        "--metastable-states", type=int, default=5, help="Number of metastable states (default: 5)"
    )

    # Example configuration generator
    example_parser = subparsers.add_parser(
        "example-config", help="Generate an example configuration file"
    )
